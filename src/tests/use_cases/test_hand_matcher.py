...
class TestHandMatcher(unittest.TestCase):
    ...
    def _assert_winner(self, expected_winner_index, hand):
        hand = [hand_string_to_hand_mapper(card) for card in hand]
        actual_winner_index = HandMatcher().compare(hand)
        self.assertEqual(actual_winner_index, expected_winner_index)
